...
func TestIntegrator(t *testing.T) {
	cases := []struct {
		...
		{Vec{0, 1}, Vec{1, 0}, 1e-1, math.Pi / 2, Vec{1, 0}, Vec{0, -1}, 3e-3},
		{Vec{0, 1}, Vec{1, 0}, 1e-2, math.Pi / 2, Vec{1, 0}, Vec{0, -1}, 3e-5},
		{Vec{0, 1}, Vec{1, 0}, 1e-3, math.Pi / 2, Vec{1, 0}, Vec{0, -1}, 3e-7},
		{Vec{0, 1}, Vec{1, 0}, 1e-4, math.Pi / 2, Vec{1, 0}, Vec{0, -1}, 3e-9},
		{Vec{0, 1}, Vec{1, 0}, 1e-5, math.Pi / 2, Vec{1, 0}, Vec{0, -1}, 3e-11},
		// even smaller steps don't increase convergence anymore due to truncation.
	}

	for i, c := range cases {
		s := NewIntegrator(UnitAcc, c.dtheta)
		s.P = c.p0
		s.V = c.v0
...
func BenchmarkIntegrate(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Integrate(AVerlet, Vec{1, 0}, Vec{0, 1}, UnitAcc, 0, 2*math.Pi, 1e-5, 1e-3)
	}
}
...
func TestAVerlet1(t *testing.T) {
	...
	for i, c := range cases {
		p, _, _ := Integrate(stepper, c.p0, c.v0, UnitAcc, 0, c.duration, 1e-6, c.dtheta)
...
func TestVerlet1(t *testing.T) {
	...
	for i, c := range cases {
		p, _, _ := Integrate(stepper, c.p0, c.v0, UnitAcc, 0, c.duration, c.dt, 0)
